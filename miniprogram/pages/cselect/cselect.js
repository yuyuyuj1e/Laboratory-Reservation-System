...
Page({
    ...
    */
    ...
    turnToConfirm: function(res) {
        ...
        // 判断实验室 预约状态
        if (this.data.laboratory_list[index].order[this.data.date][this.data.time].class_status == '不可预约') {
        ...
        // 判断 实验室 可人数是否 是否满足要求
        else if (this.data.count > this.data.laboratory_list[index].order[this.data.date][this.data.time].now_count) {
            console.log('实验室空位不足')
            wx.showToast({
                title: '实验室空位不足，请预约其他实验室',
                icon: 'error'
            })
        }
        // 已经预约过该实验室
        else if(this.data.laboratory_list[index].order[this.data.date][this.data.time].class_status == '已预约') {
    ...
    onLoad: function (options) {
        ...
        db.collection('category').where({classification: db.command.neq(null)}).get().then( res => {
            ...
            let list = status_util.getStatusClass(this.data.laboratory_list, this.data.appointment_record_list, this.data.date, this.data.time)
            this.setData({
